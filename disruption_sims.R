...
disruption_sims <- lapply(1:length(gamma), function(z){
  ...
  base <- model(pop_size = n, t = t, neg_cost = cost[z], n_moves = moves[z], gamma = gamma[z], f = f[z], networked = networked[z], last_output = TRUE)
  status_quo <- which.max(as.numeric(table(factor(base$status_quo, levels = 1:moves[z]))))

  #get frequency of previous status quo
  base_freqs <- sapply(1:t, function(y){length(which(base$status_quo == status_quo))/n})
