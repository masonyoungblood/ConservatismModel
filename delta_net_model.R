...
model_slurm <- function(neg_costs, n_moves){
  #lapply(1:reps, function(x){
    model(pop_size = pop_size, t = t, priors = c(1, 0, 0, 0), neg_cost = neg_costs, n_moves = n_moves, phi = 1, delta = 1, kappa = 0, lambda = 1, networked = TRUE)
  #})
}

#run simulations
rslurm::slurm_apply(model_slurm, params, jobname = "delta_net_model",
